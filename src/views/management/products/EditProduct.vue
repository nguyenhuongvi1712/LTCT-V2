<template>
  <h2 class="text-center mb-4">Edit product (Id : {{ this.id }})</h2>
  <CForm @submit.prevent="handleSubmit" v-loading="loading">
...
<script>
import { editProduct, getListCategory, getProductById } from '@/api/products'
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      const res = await editProduct(this.id, req)
